...
//|                                             Powerd by Gemini AI |
//|------------------------------------------------------------------|
//| نسخه 5.3 - فیکس نهایی مدیریت آرایه FVG و ابطال لحظه‌ای (Ask/Bid) |
//| 1. رفع باگ زمان 1970 با استفاده صحیح از ArrayInsert.             |
//| 2. حذف قطعی FVG ابطال شده از آرایه برای جلوگیری از لاگ تکراری.   |
//| 3. تأیید اندیس FVG (کندل 2 به عنوان کندل میانی).                |
//| 4. عدم جستجوی ساختار اولیه در تمام تاریخچه چارت (بهینه).         |
//+------------------------------------------------------------------+
...
void AddFVG(bool isBullish, double highPrice, double lowPrice, datetime time) 
{
   ...
   // ✨ اصلاح اساسی: استفاده از ArrayInsert برای افزودن عنصر جدید در ابتدای آرایه سری (اندیس 0)
   // این کار مشکل زمان 1970 را که ناشی از عدم مقداردهی صحیح در ArrayResize بود، حل می‌کند.
   if(ArrayInsert(fvgArray, 0, 1) == -1) 
   {
...
bool CheckFVGInvalidation() 
{
   ...
   for(int i = 0; i < ArraySize(fvgArray); i++) 
   {
      ...
      if(isInvalidated)
      {
         ...
         // ✨ حذف عنصر ابطال شده از آرایه برای جلوگیری از چک کردن مجدد و مشکل 1970
