...
// شامل فایل‌های لازم برای کار با اشیاء گرافیکی و توابع MQL5 - برای دسترسی به ثابت‌هایی مثل CORNER_RIGHT_UPPER
#include <Object.mqh>
...
public:
   ...
   bool              IsBOS() { return m_isBOS; }          // آیا BOS اخیر رخ داده
   bool              IsCHOCH() { return m_isCHOCH; }      // آیا CHOCH اخیر رخ داده
      ...
      if(m_enableGraphics)               // اگر گرافیک فعال باشد، رسم خط شکست
        {
         ...
         DrawDottedLine(m_objPrefix + "Break_" + TimeToStr(toTime), fromTime, m_isUpwardBreak ? m_lastHigh.price : m_lastLow.price, toTime, closePrice, label); // رسم خط نقطه‌چین با متن
        }
     }
         ...
         if(m_isUpwardBreak)             // اگر شکست صعودی بوده، ثبت سقف جدید
           {
            ...
            if(m_enableGraphics) DrawSwing(m_objPrefix + "High_" + TimeToStr(m_lastHigh.time), m_lastHigh.price, m_lastHigh.time, clrGreen, "SH"); // رسم SH اگر گرافیک فعال
           }
         else                            // اگر شکست نزولی بوده، ثبت کف جدید
           {
            ...
            if(m_enableGraphics) DrawSwing(m_objPrefix + "Low_" + TimeToStr(m_lastLow.time), m_lastLow.price, m_lastLow.time, clrRed, "SL"); // رسم SL اگر گرافیک فعال
           }
   ...
   if(isUpward)                          // اگر شکست صعودی باشد، آپدیت کف (لو) به عنوان نقطه مقابل با پایین‌ترین لو در بازه
     {
      ...
      m_lastLow.price = iLow(m_symbol, m_timeframe, lowest); // آپدیت قیمت کف بدون اضافه به آرایه
   ...
   else                                  // اگر شکست نزولی باشد، آپدیت سقف (های) به عنوان نقطه مقابل با بالاترین های در بازه
     {
      ...
      m_lastHigh.price = iHigh(m_symbol, m_timeframe, highest); // آپدیت قیمت سقف بدون اضافه به آرایه
...
void StructureAnalyzer::RemoveOldObjects()
  {
   ...
   while(ArraySize(m_highs) > m_maxStructures)
     {
      string oldName = m_objPrefix + "High_" + TimeToStr(m_highs[0].time); // نام شیء قدیمی سقف بر اساس زمان
   ...
   while(ArraySize(m_lows) > m_maxStructures)
     {
      string oldName = m_objPrefix + "Low_" + TimeToStr(m_lows[0].time); // نام شیء قدیمی کف بر اساس زمان
      ObjectDelete(0, oldName);             // پاک کردن شیء قدیمی از چارت 0
