...
ببینید ما می‌خواهیم یک تمیزکاری روی کد زیر انجام رنگ تمام اشیایی که ما روی چارت رسم کنیم یک بخش در واقع در ابتدای کد قرار بدیم که اینجا این رنگ‌ها رو بشه در حالا در ابتدا وجود داشته باشه که اگه خواسته باشم تغییر بدیم این‌ها رو رنگاشون تغییر بدیم و دیگه نیاز نباشه تو کل کد دنبال این رنگ‌ها بگردم هر کدوم از این رنگ‌ها مثلاً رنگ اف جی صعودی اینجوری مشخص کنید برای بقیه هم همینطور 
و خوب با کامنتم یک ناحیه رو جدا کنید که مشخص باشه بعدش می‌خوایم که در مورد لاک‌های سیستم رو کمی خیلی خیلی بهبود ببخشیم
لاک‌ها رو باید ما سطح‌بندی کنیم در وهله اول بیایم در سه سطح در واقع لاگ رو می‌خواهیم تنظیم کنیم یک لاک به نام لاگ‌های فول لاگ هستش که تمام لاک‌ها رو چاپ خواهد کرد یک لایک هست به نام لاک‌های عملکردی و یک لایک هم داریم به نام لاک خطا لاک لاک‌های خطا
خوب لاک‌های فول لاک که دیگه شامل همه می‌شه یعنی هر حرکت هر اتفاقی چه خطایی چه هر واکنشی که در هر نوع محاسبه‌ای که انجام هر چیزی جاهایی که الان در لاگی نداری در سیستم با همه جا لاگ گذاشته بشه هیچ جایی از سیستم نباید بدون لاک بمونه وقتی در این حالت قرار می‌دیم همه چی حالت‌ها اتفاق می‌افته 
در حالت عملکردی اینجا ببینید در واقع هر عمل مهمی که انجام می‌شه ببینید وقتی که یک سقف و یک کف در واقع پیدا می‌شه و پیدا میشه یا باطل می‌شه یا یک ارتوپلاک پیدا می‌شه یا باطل می‌شه یا هر اتفاقایی عملکرد داشته باشیم باید در لاک داشته باشیم بلکه ما نباید در لاگ عملکرد نباید هی تکرار انجام بشه نباید در هر تیک یک لاک پشت سر هم چاپ بشه به هیچ وجه باید هر اتفاقی که می‌افته در واقع وقتی که یک حالت کار خاص انجام می‌شه این ثبت بشه و لاک هم مرتب و تمیز دیده بشه و به صورت مثلاً لمات کلیدی وار نوشته شده باشه و اینجوری باشه که آقا وقتی که داریم می‌نویسیم سمت اتفاق باشه بعد یک فاصله‌ای بخوره و بعد زمان واقع و نوع واقعه یا هر چیزی که داره لاک مثل یه جدول خوانا و قشنگ نمایش داده بشه در حالتی که لاک روی حالت خطا باشه فقط باید خطاها رو نشون بده یعنی هر جایی که خطا ایجاد بشه و در کد هم باید شما بیاین جاهایی که می‌تونه خطا باشه یعنی حالت‌های مختلفی برای لاک تعریف کنید که خطا رو برگردونه و این خطاها رو داشته باشیم تمام حالت‌ها و خطاهایی که می‌تونه ایجاد بشه رو باید پوشش بده و در این بخش حالا خطاها بتونه هنگامی که رینگ باشه گزارش بده یه خطایی باشه یا هشداری باشه یا هر چیزی 
بعدش باید تمام لاک‌هایی که ما سیستم استفاده می‌کنیم چه لاک‌هایی عمل کردی چه لاک‌هایی مخصوصاً لاک‌های خطا باید دارای کد باشند خطا برای خطا حالا خطای ۱۰۱ خطای ۱۰۵ خطای ۷۰۵ در مورد لایوهای عملکردی و اتفاقات مهم هم دارای کد باشند این کد باید وجود داشته باشه حالا متن خطا نوشته می‌شه ولی کد هم داخلش می‌نویسه و دیگه آقا یک سیستم لاگ خیلی حرفه‌ای و تمیز می‌خوام مرتب حرفه‌ای و فوق العاده کامل می‌خوام دیگه خودت ببین یک لای خوب و قشنگ کامل که حالت‌های مختلف داشته باشه طبقه‌بندی شده باشه یه چیز تجاری و خفن می‌خوام دیگه خودت ببین هرچی لازمه ریش و قیچی دست خودت خودرو هم در زیر می‌ذارم لطفاً این کد رو با توجه به توضیحاتی که گفتم و چیزهایی که فکر می‌کنید لازم هست اصلاح کن و کد کامل و حساس شده را به شکل نهایی و کامل مجدد بازنویسی و ارسال خواهید کرد 
